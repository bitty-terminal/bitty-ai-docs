...
> proposed in [AI Architecture](ai-architecture.md) under
...
[AI Architecture](ai-architecture.md), the R1 through R6 draft dispositions
in [Execution ownership R1](execution-ownership-r1.md),
...
[Task lifecycle R5](task-lifecycle-r5.md), and
[Persistence profile R6](persistence-profile-r6.md), the narrow scope gate in
[v0.1 Implementation Profile](implementation-profile-v0.1.md), the gap wording
in [AI Vertical Slice Pressure Test](ai-vertical-slice-pressure-test.md), the
register in [AI Unresolved Questions](ai-unresolved-questions.md), the handoff
...
prompt facet in [Prompt Layering Design](prompt-layering-design.md), the
stable-prefix facet in
[Prefix-Cache-Friendly Context Design](prefix-cache-context-design.md), and the
accepted [IPC and Agent RFC](ipc-agent-rfc.md) as overriding authority.
...
isolation) in [AI Architecture](ai-architecture.md); the R1 registry-split
framing in [Execution ownership R1](execution-ownership-r1.md); the
...
testability) in [AI Architecture](ai-architecture.md);
[Context management architecture](context-management.md);
[v0.1 Implementation Profile](implementation-profile-v0.1.md) Levels 0 and 1;
the R3 retention floor in [Context retention R3](context-retention-r3.md).
...
[AI Architecture](ai-architecture.md); the single-agent baseline in
[Execution ownership R1](execution-ownership-r1.md); the P1 bridge module in
...
(Budgets and backpressure) in [AI Architecture](ai-architecture.md); the
unified-backend disposition in [Tool transport R2](tool-transport-r2.md); the
cancellation contract in [Execution ownership R1](execution-ownership-r1.md);
...
Draft sources: [Prompt Layering Design](prompt-layering-design.md);
[Prefix-Cache-Friendly Context Design](prefix-cache-context-design.md);
AG-4 (Least privilege at dispatch) in [AI Architecture](ai-architecture.md);
...
- [AI Architecture](ai-architecture.md) (Draft): umbrella with the CTX-0023
...
- [Tool transport R2](tool-transport-r2.md) (Draft): unified-backend draft
  disposition and fail-closed evidence bar.
- [Context retention R3](context-retention-r3.md) (Draft): consent-bounded
...
- [Prompt Layering Design](prompt-layering-design.md) (Draft): five-layer
