...
[AI Architecture](ai-architecture.md), the draft storage direction in
...
- **R1 (host owns execution).** History never authorizes execution.
  [Execution ownership R1](execution-ownership-r1.md) keeps
...
- **R2 (unified authorization).** Every history read passes the
  [Tool transport R2](tool-transport-r2.md) gate order: authenticated
...
- **R3 floor (redaction and consent).** P0-AC-026 with PP-2 (Typed redaction)
  ...
  [AI Architecture](ai-architecture.md) are mandatory and not reopened. The
  [Context retention R3](context-retention-r3.md) floor applies verbatim:
...
[AI Unresolved Questions](ai-unresolved-questions.md) and shared governance
