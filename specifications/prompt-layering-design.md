...
which elaborate [AI Architecture](ai-architecture.md) CP-5 (Budget), CP-6 (Artifacts),
and CP-7 (Determinism and testability). Capability separation is constrained by
AG-4 (Least privilege at dispatch) in [AI Architecture](ai-architecture.md) and the
Core-versus-Lua boundary in [Command and Tool Architecture](command-tool-architecture.md).
...
- Panel and execution lifecycle semantics stay with
  [Agent Coordination Architecture](agent-coordination.md) (`Agent -> ExecutionContext <- Panel`)
  and the accepted [IPC and Agent RFC](ipc-agent-rfc.md). This proposal treats
...
- Skill format and versioning stay with AIQ-09; instruction snapshots and
  ...
  [AI Architecture](ai-architecture.md); progressive `describe` and `resolve`
...
[Agent Coordination Architecture](agent-coordination.md) only after the v0.1
...
[AI Architecture](ai-architecture.md) ("Lua owns ... prompt assembly");
...
[AI Architecture](ai-architecture.md). Any future introspection surface must
...
Consistent with [v0.1 Implementation Profile](implementation-profile-v0.1.md)
...
- [AI Architecture](ai-architecture.md) (Draft): CP-5 (Budget), CP-6 (Artifacts),
