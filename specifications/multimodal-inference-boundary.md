...
[AI Unresolved Questions](ai-unresolved-questions.md), the narrow scope gate in
[v0.1 Implementation Profile](implementation-profile-v0.1.md), and the accepted
[IPC and Agent RFC](ipc-agent-rfc.md) as overriding authority. The source
...
vocabulary (MP-2, MP-4 in [AI Architecture](ai-architecture.md)) with
...
exactly-once claim (MP-7 (`cancel`) in [AI Architecture](ai-architecture.md),
the cancellation contract in
[Execution ownership R1](execution-ownership-r1.md), and the R2 gate order in
...
(Artifacts) drill-down contract in [AI Architecture](ai-architecture.md),
...
the optional-Projection rule from [Execution ownership R1](execution-ownership-r1.md)
...
in [Tool transport R2](tool-transport-r2.md): Lua selects tools and tasks,
...
- R1 red line preserved. The R1 disposition in
  [Execution ownership R1](execution-ownership-r1.md) keeps the rule that
...
- R2 unified authorization backend preserved. The R2 disposition in
  [Tool transport R2](tool-transport-r2.md) keeps one common authorization
...
- MP-10 (API-key handling) not reopened. Storage, redaction, consent
  ...
  [AI Architecture](ai-architecture.md); multimodal adapters sit behind the
...
[v0.1 Implementation Profile](implementation-profile-v0.1.md). Nothing in
...
[AI Unresolved Questions](ai-unresolved-questions.md) finds every boundary
