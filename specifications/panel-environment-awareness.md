...
[Context retention R3](context-retention-r3.md), the R6 disposition in
[Persistence profile R6](persistence-profile-r6.md), the register in
[AI Unresolved Questions](ai-unresolved-questions.md), the narrow scope gate
in [v0.1 Implementation Profile](implementation-profile-v0.1.md), and the
accepted [IPC and Agent RFC](ipc-agent-rfc.md) as overriding authority. The
...
[AI Architecture](ai-architecture.md#privacy-first) stay mandatory; typed
`SecretField` redaction applies pre-queue and pre-write, and the secret
invariant in
[Provider plugin boundary](provider-plugin-boundary.md#secret-invariant)
...
[Execution ownership R1](execution-ownership-r1.md) (no model selection or
...
backend in [Tool transport R2](tool-transport-r2.md) (one common gate order
...
process model) in [AI Architecture](ai-architecture.md), the bridge
identity and consent seam in the accepted
[IPC and Agent RFC](ipc-agent-rfc.md), and the declared-placement rule in
[Tool transport R2](tool-transport-r2.md). Loading or naming a snapshot
...
in [Context retention R3](context-retention-r3.md) and the R6 backend and
release-profile disposition in
[Persistence profile R6](persistence-profile-r6.md), under PP-4 (No on-disk
persistence without consent) and P0-AC-026 in
[AI Architecture](ai-architecture.md#privacy-first).
...
[Command and tool architecture](command-tool-architecture.md) and the
projection-only rule in
[Execution ownership R1](execution-ownership-r1.md) (presentation movement
...
- R2 unified authorization backend preserved. Environment use, snapshot
  ...
  attributed outcome) in [Tool transport R2](tool-transport-r2.md).
...
- R3 retention and R6 deferral preserved. No durability, replay, or release
  ...
  [Context retention R3](context-retention-r3.md) or the R6 baseline in
...
- MP-10 handling not reopened. Storage, redaction, consent separation, and
  ...
  [AI Architecture](ai-architecture.md); environment-derived secrets inherit
  the same opaque-handle rule as provider credentials.

## Open choices and identifier check

Duplicate-check against
[AI Unresolved Questions](ai-unresolved-questions.md) finds every awareness
