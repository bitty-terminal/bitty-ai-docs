...
**Status**: Optional projection is a draft integration with the accepted Panel lifecycle, not a replacement state machine. [Agent coordination](agent-coordination.md#panel-model-reconciliation) retains `Agent -> ExecutionContext <- Panel`; no-panel execution does not imply a persistent terminal daemon or new transport authority.
