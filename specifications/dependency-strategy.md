...
[AI Architecture](ai-architecture.md) MP-3 (Local-first default), TB-1 (MCP as
...
execution). It is gated by the [v0.1 Implementation Profile](implementation-profile-v0.1.md)
...
[Code Intelligence Architecture](code-intelligence.md), and
[Persistence and Evidence Architecture](persistence-evidence.md). These are
...
- Provider registry, descriptor, consent, budget, and credential handling stay
  with [AI Architecture](ai-architecture.md) MP-1 through MP-11, especially
...
- Tool Bus validation, consent, budgets, and host-only execution stay with
  [AI Architecture](ai-architecture.md) TB-1 (MCP as adapter) through TB-7 and
  with [Command and Tool Architecture](command-tool-architecture.md). This
...
- Context budget, artifacts, and determinism stay with
  [AI Architecture](ai-architecture.md) CP-5 (Budget), CP-6 (Artifacts), and
  CP-7 (Determinism and testability), elaborated by
  [Context Management Architecture](context-management.md) and
  [Prefix-Cache-Friendly Context Design](prefix-cache-context-design.md). This
...
- Service supervision, Panel and execution ownership, and delegation budgets
  stay with [Agent Coordination Architecture](agent-coordination.md) under
...
- LSP sharing, verification fingerprinting, and lint/build/test reuse stay
  with [Code Intelligence Architecture](code-intelligence.md). This proposal
...
- Journal representation, backend selection, retention, and replay semantics
  stay with [Persistence and Evidence Architecture](persistence-evidence.md).
  This proposal selects no backend and defines no schema (AIQ-51 through
  AIQ-5C).
- The v0.1 scope gate stays with the
  [v0.1 Implementation Profile](implementation-profile-v0.1.md). Anything not
  listed there is out of scope for v0.1, not rejected.
- Open-question ownership and promotion stay with
  [AI Unresolved Questions](ai-unresolved-questions.md) and the canonical OQ
...
eligibility stay with [Code Intelligence Architecture](code-intelligence.md)
...
[AI Architecture](ai-architecture.md). P0 trust boundaries stay release
blockers.

## v0.1 scope boundary

Consistent with the [v0.1 Implementation Profile](implementation-profile-v0.1.md)
...
Duplicate-check outcome against [AI Unresolved Questions](ai-unresolved-questions.md):
...
- [AI Architecture](ai-architecture.md) (Draft): MP-3 (Local-first default),
...
- [Code Intelligence Architecture](code-intelligence.md) (Draft): broker,
  fingerprint, and reuse prerequisites AIQ-41 through AIQ-48.
- [Persistence and Evidence Architecture](persistence-evidence.md) (Draft):
  representation, backend, retention, and replay prerequisites AIQ-51 through
  AIQ-5C.
- [AI Unresolved Questions](ai-unresolved-questions.md) (Draft): AIQ-08,
