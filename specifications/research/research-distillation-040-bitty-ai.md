...
on 2026-09-16: **1,829 lines**, **31,779 bytes**. Its SHA-256 is
...
The expected output is the SHA-256 above with 1,829 lines and 31,779 bytes.
...
Both must print the SHA-256 above.
...
### Post-verification append (recorded, not distilled)

After the task-time verification above, the source file grew by a pure
append: at 2026-09-16 the file held 2,215 lines and 42,579 bytes, while the
first 1,829 lines (31,779 bytes) still hash to exactly the verified SHA-256
above, and spot-checked anchors (`040.md:138`, `229`, `402`, `437`) are
unchanged. The appended tail (`040.md:1830` onward, roughly 386 lines, a
comparable-programs discussion covering Emacs, VS Code, JupyterLab, Eclipse
RCP, and similar references) was skimmed for exclusion accuracy only: it was
**not** distilled, adds no cited range to this document, and remains
uncovered by both this distillation and the coverage ledger. A follow-up
task is needed if the commander wants the appended round distilled; until
then the whole-file hash differs from the provenance fingerprint while every
range cited here still verifies against it.
