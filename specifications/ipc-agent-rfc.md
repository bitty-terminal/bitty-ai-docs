...
Bitty IPC does not copy a wire format from prior art. Hyprland, Waybar, and analogous references are non-normative research leads: every Hyprland-inspired behavior would need to be re-expressed as a typed, validated contract, and no Hyprland or Waybar source, configuration syntax, or wire format is vendored (see [Workspace Compositor](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/specifications/workspace-compositor.md) Hyprland import rules). The lineage below is design intuition and provenance, not an accepted contract.
