...
> The accepted [IPC and Agent RFC](ipc-agent-rfc.md) and the normative security
...
boundary](provider-plugin-boundary.md) (the `Router` transport at `:130-132`,
the two-level plugin model at `:141-166`, the secret invariant at `:194`), and
the accepted [IPC and Agent RFC](ipc-agent-rfc.md) as overriding authority.
...
plugin boundary](provider-plugin-boundary.md) (`:130-132`): a multi-upstream
...
boundary](provider-plugin-boundary.md), `:115-132`) wire the field when such
...
1. Identity is not credential. The secret invariant in [Provider plugin
   boundary](provider-plugin-boundary.md) (`:194`) applies unchanged:
...
boundary](provider-plugin-boundary.md), `:141-166`: a Model Manager UI and
...
RFC](ipc-agent-rfc.md) keeps its normative framing, scope, lifecycle, and
consent statements, and no sibling draft disposition ([Provider plugin
boundary](provider-plugin-boundary.md) included) is revised by this
...
- [AI Unresolved Questions](ai-unresolved-questions.md) (Draft): no entry
