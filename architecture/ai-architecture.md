...
Status: **candidate, non-normative**. This subsection extends MP-10 without changing it. Configuration shape, transport kinds, credential references, alias resolution, and the secret invariant are owned by [Provider plugin boundary](../providers/provider-plugin-boundary.md); unresolved parts stay tracked as [OQ-054, OQ-055](https://github.com/bitty-terminal/bitty-docs/blob/main/docs/decisions/open-questions.md), and the storage tiers are recorded in the [Plugin Roadmap](https://github.com/bitty-terminal/bitty-plugins-docs/blob/main/product/plugin-roadmap.md) secrets direction. Mandates that stay in force:

- **MPC-1 Provider entries.** Provider kinds (`openai_compatible`, `anthropic`, `ollama`) are transport adapters, never capability grants: a remote kind still requires the accepted `network.connect` grant and `ai.provider` consent, and a `local-only` provider performs no network I/O (MP-3).
...
The repository-index layer separation (file, syntax, symbol, import, semantic,
search), language-service discovery, semantic tool surface, and verification
reuse are owned by
[Code intelligence architecture](../agent/code-intelligence.md); the mandates
below stay in force:
...
- **TB-4 Capability and consent per tool.** Using a tool requires both its declared capability (for example `workspace.write`, `git.read`, `diagnostics.read`, `terminal.inspect`) and a separate per-client consent grant ledgered as `who, agent_id, tool_name, granted_at, expires_at, granted_by`; possession of `ai.model.stream` implies no tool authority.
