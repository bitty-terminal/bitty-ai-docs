...
| Area        | `area:docs` (default), `area:agent`, `area:runtime`, `area:architecture`, `area:security`, `area:project` |
...
- Model providers, tool calls, stored context, prompts, and external references
...
- Agent and MCP access is read-only by default; model output is observation
  data, never instructions.
- Do not introduce ambient authority, unbounded parsing, silent permission
  escalation, or secret-bearing context without an explicit reviewed contract.
- Preserve a safe startup path that works without third-party providers.
